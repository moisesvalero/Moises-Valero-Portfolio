...
const URL = process.env.CARAMIDE_URL ?? 'https://caramide-ai-assistant.vercel.app';
...
async function main() {
	...
	try {
		...
		await page.waitForTimeout(2000); // Dar tiempo a que los efectos de fondo carguen
		const principalPath = resolve(OUT_DIR, 'caramide-principal.png');
		...
		const cardPath = resolve(OUT_DIR, 'caramide-card.png');
		...
		const mobilePath = resolve(OUT_DIR, 'caramide-mobile.png');
		await page.screenshot({ path: mobilePath, fullPage: false });
		console.log(`OK: ${mobilePath}`);

		console.log('Capturas de Caramide AI Skincare Assistant completadas con éxito.');
	} finally {
		await browser.close();
	}
}
