...
const URL = process.env.RECETARIO_URL ?? 'https://recetario.moisesvalero.es';
...
async function main() {
	...
	try {
		...
		const principalPath = resolve(OUT_DIR, 'recetario-ia-principal.png');
		...
		const cardPath = resolve(OUT_DIR, 'recetario-ia-card.png');
		...
		const mobilePath = resolve(OUT_DIR, 'recetario-ia-mobile.png');
		await page.screenshot({ path: mobilePath, fullPage: false });
		console.log(`OK: ${mobilePath}`);

		console.log('Capturas de Recetario IA completadas con éxito.');
	} finally {
		await browser.close();
	}
}
