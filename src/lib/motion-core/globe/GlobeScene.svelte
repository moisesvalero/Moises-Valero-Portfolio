<script lang="ts">
	...
	onMount(() => {
		...
		try {
			...
			const markerData = new Float32Array(MAX_SHADER_MARKERS * 4);
			const markerColorData = new Float32Array(MAX_SHADER_MARKERS * 3);
		...
		`;
			...
			console.warn('[GlobeScene] Scene setup failed, falling back:', err);
			webGlSupported = false;
		}
	});
</script>
