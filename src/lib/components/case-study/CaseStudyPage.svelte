...
<div class="case-study-root">
	...
	<div class="cta-section">
		<div class="cta-actions">
			<a href={study.liveUrl} class="btn" target="_blank" rel="noopener noreferrer"
				>{L.verProyectoVivo}</a
			>
			{#if study.repoUrl}
				<a href={study.repoUrl} class="btn btn-github" target="_blank" rel="noopener noreferrer"
					>{L.verRepositorio}</a
				>
			{/if}
...
<style>
	.case-study-root {
		...
		background: linear-gradient(180deg, rgba(232, 240, 249, 0.5), transparent 24rem), var(--fondo);
		color: var(--principal);
		width: 100%;
	...
	:global(html.dark) .case-study-root {
		...
		background:
			radial-gradient(circle at 50% 0%, rgba(0, 113, 227, 0.14), transparent 36rem), var(--fondo);
	}
