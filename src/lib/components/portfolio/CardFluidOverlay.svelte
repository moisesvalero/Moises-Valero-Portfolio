<script lang="ts">
	...
	let { color = '#0066E5', strength = 0.22, radius = 220, blur = 32 }: Props = $props();
...
<style>
	.card-fluid-overlay {
		...
		background: radial-gradient(
			...
			color-mix(
					in srgb,
					var(--cf-color, #0066e5) calc(var(--cf-strength, 0.22) * 100%),
					transparent
				)
				0%,
			color-mix(in srgb, var(--cf-color, #0066e5) calc(var(--cf-strength, 0.22) * 45%), transparent)
				38%,
			transparent 70%
		);
		filter: saturate(112%);
