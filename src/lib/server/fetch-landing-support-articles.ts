...
function mapRow(
	...
	const imageFromAsset =
		ctx && ctx.projectId && ctx.dataset
			? imageUrl(ctx.projectId, ctx.dataset, row.image, 1200)
			: undefined;
...
export async function fetchLandingSupportArticles(
	limit?: number
	...
	try {
		const rows = await client.fetch<LandingSupportArticleListRow[] | null>(
			landingSupportArticlesQuery
		);
		const mapped = (rows ?? [])
