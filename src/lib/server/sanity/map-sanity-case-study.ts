...
function section(
	...
	const bodyHtml = chosenBody || `<p>(${key})</p>`;
...
export function mapSanityRowToCaseStudy(
	...
	const principalFromAsset =
		ctx && ctx.projectId && ctx.dataset
			? imageUrl(ctx.projectId, ctx.dataset, img.principalImage, 1600)
			: undefined;
	const secondary1FromAsset =
		ctx && ctx.projectId && ctx.dataset
			? imageUrl(ctx.projectId, ctx.dataset, img.secondary1Image, 1200)
			: undefined;
	const secondary2FromAsset =
		ctx && ctx.projectId && ctx.dataset
			? imageUrl(ctx.projectId, ctx.dataset, img.secondary2Image, 1200)
			: undefined;
	...
	const stack =
		locale === 'en' && Array.isArray(row.stackEn) && row.stackEn.length
			...
			: Array.isArray(row.stack)
				...
				: [];
	const metricsSource =
		locale === 'en' && Array.isArray(row.metricsEn) ? row.metricsEn : row.metrics;
	...
	return {
		...
		heroDescription:
			chooseString(locale, row.heroDescription, row.heroDescriptionEn) || row.heroDescription,
		tags,
		images: {
			principal,
		...
		hice: section(
			...
			row.hice,
			row.hiceEn,
			locale === 'en' ? 'What I built' : 'Lo que hice'
		),
		resultado: section(
			'resultado',
			locale,
