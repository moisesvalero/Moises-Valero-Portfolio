<script lang="ts">
	...
	const animatedOverallScore = $derived(
		result ? Math.round(result.overallScore * animatedProgress) : 0
	);
	...
	const criticalIssues = $derived(
		result?.issues.filter((item) => item.severity === 'critical').length ?? 0
	);
	const warningIssues = $derived(
		result?.issues.filter((item) => item.severity === 'warning').length ?? 0
	);
	const infoIssues = $derived(
		result?.issues.filter((item) => item.severity === 'info').length ?? 0
	);
			...
			? [
					{
						label: 'Rendimiento',
						score: Math.round(
							(result.categoryScores.performance || result.performanceScore) * animatedProgress
						),
						checks: [
					...
					},
					{
						...
						checks: [
							`${result.signals.imagesWithoutAlt} imágenes sin alt`,
							`${result.signals.smallTapTargets} tap targets`,
							`${result.signals.lowContrastTexts} contrastes`
						]
					}
				]
			: []
	...
	function normalizeSignals(input: Partial<AnalyzerResult>['signals']): AnalyzerResult['signals'] {
		return {
			...
			brokenInternalLinks:
				typeof input?.brokenInternalLinks === 'number' ? input.brokenInternalLinks : 0,
			estimatedResourceBytes:
				typeof input?.estimatedResourceBytes === 'number' ? input.estimatedResourceBytes : 0,
			detectedTechnologies: Array.isArray(input?.detectedTechnologies)
				? input.detectedTechnologies.filter(
						(item): item is string => typeof item === 'string' && item.trim().length > 0
					)
				: [],
			wordPressPlugins: Array.isArray(input?.wordPressPlugins)
				? input.wordPressPlugins.filter(
						(item): item is string => typeof item === 'string' && item.trim().length > 0
					)
				: [],
			...
			visualAuditReason:
				typeof input?.visualAuditReason === 'string'
					? input.visualAuditReason
					: 'Auditoría visual no ejecutada.',
			...
			renderedResourceErrors:
				typeof input?.renderedResourceErrors === 'number' ? input.renderedResourceErrors : 0,
			...
			horizontalOverflowViewports: Array.isArray(input?.horizontalOverflowViewports)
				? input.horizontalOverflowViewports.filter(
						(item): item is string => typeof item === 'string' && item.trim().length > 0
					)
				: [],
			brokenRenderedImages:
				typeof input?.brokenRenderedImages === 'number' ? input.brokenRenderedImages : 0,
			cookiesBeforeConsent:
				typeof input?.cookiesBeforeConsent === 'number' ? input.cookiesBeforeConsent : 0,
			viewportChecks: Array.isArray(input?.viewportChecks)
				? input.viewportChecks.filter(
						(item): item is string => typeof item === 'string' && item.trim().length > 0
					)
				: [],
	...
	async function pollAnalyzeJob(
		...
		return {
			ok: false,
			error: 'El análisis está tardando demasiado. Prueba de nuevo en unos minutos.'
		};
	}
...
<main class="tool-page">
	<section class="tool-hero">
		...
		<p class="lead">
			Analiza una URL en una sola pasada: rendimiento, seguridad visible, SEO técnico,
			accesibilidad, cabeceras HTTP, WordPress y señales sospechosas.
		</p>
	</section>
	...
	{#if status === 'loading'}
		...
		>
			...
			<div class="loading-copy">
				...
				<p>
					Estoy midiendo rendimiento, cabeceras, SEO técnico, accesibilidad y señales visibles antes
					de montar el informe.
				</p>
			</div>
	...
	{:else if result}
		<section
			class={`hero-results ${scoreClass}`}
			aria-label="Resultado del analizador"
			aria-live="polite"
		>
			<div class="global-score">
				<div
					class="global-score-ring"
					style={scoreRingStyle}
					aria-label={`Puntuación global: ${scoreLabel}`}
				>
				...
				{#each heroCards as card (card.label)}
					<article class={`metric-card ${scoreToneClass(card.score)}`}>
						...
						<div class="metric-card-body">
							<div
								class="mini-ring"
								style={cardRingStyle(card.score)}
								aria-label={`${card.label}: ${card.score}%`}
							>
			...
			>
				<div>
					...
					<p>
						Te envío los 10 apartados, puntuaciones, problemas prioritarios y acciones recomendadas.
					</p>
				</div>
				<input
			...
			<details class="full-report" open>
				...
				<div class="source-grid" aria-label="Senales tecnicas detectadas">
					...
					<div>
						...
						<strong
							>{result.signals.resourceCount} recursos - {result.signals.resourceErrors} errores</strong
						>
					</div>
					<div>
						<span>Enlaces internos</span>
						<strong
							>{result.signals.internalLinks} detectados - {result.signals.brokenInternalLinks} rotos</strong
						>
					</div>
					<div>
					...
					<div>
						<span>404</span>
						<strong
							>{result.signals.hasCustom404 ? 'Personalizada detectada' : 'No detectada'}</strong
						>
					</div>
					<div>
						<span>Tecnologias</span>
						<strong
							>{result.signals.detectedTechnologies.length
								? result.signals.detectedTechnologies.slice(0, 4).join(', ')
								: 'Sin fingerprint claro'}</strong
						>
					</div>
				</div>
				...
				<div class="source-grid" aria-label="Auditoria visual con navegador">
					<div>
						...
						<strong
							>{result.signals.visualAuditAvailable
								? `Ejecutado: ${result.signals.viewportChecks.join(', ') || 'viewport real'}`
								: result.signals.visualAuditReason}</strong
						>
					</div>
					<div>
						<span>Consola JS</span>
						<strong
							>{result.signals.consoleErrors} errores - {result.signals.consoleWarnings} avisos</strong
						>
					</div>
					<div>
						<span>Render resources</span>
						<strong
							>{result.signals.renderedResourceErrors} respuestas rotas - {result.signals
								.failedRequests} requests fallidas</strong
						>
					</div>
					<div>
						<span>Responsive real</span>
						<strong
							>{result.signals.horizontalOverflowViewports.length
								? `Overflow: ${result.signals.horizontalOverflowViewports.join(', ')}`
								: 'Sin overflow detectado'}</strong
						>
					</div>
					<div>
						<span>Accesibilidad visual</span>
						<strong
							>{result.signals.smallTapTargets} tap targets - {result.signals.lowContrastTexts} contrastes</strong
						>
					</div>
					<div>
													...
													<div class="repair-box">
														...
														{#if item.aiPrompt}
															<button
																...
																onclick={() => void copyIssuePrompt(item)}
															>
