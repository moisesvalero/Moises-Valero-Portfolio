                  ...
                  ${
										issue.affectedResources.length
											? `<p style="margin:0 0 6px;color:#0f172a;font-size:13px;"><strong>Detectado en:</strong><br>${issue.affectedResources
													...
													.map(
														(resource) =>
															`<code style="font-size:12px;word-break:break-all;">${toSafeHtml(resource)}</code>`
													)
													.join('<br>')}</p>`
											: ''
									}
...
function issuePlainText(issue: EmailIssue, index: number): string {
	const lines = [
		...
		issue.repairSteps.length
			? `   Pasos: ${issue.repairSteps.map((step, stepIndex) => `${stepIndex + 1}) ${step}`).join(' ')}`
			: `   Como arreglarlo: ${issue.fix}`
	];
...
export const POST: RequestHandler = async ({ request, url: requestUrl, getClientAddress }) => {
	...
	if (now - lastSentAt < LEAD_COOLDOWN_MS) {
		return json(
			{ ok: false, error: 'Espera unos segundos antes de volver a enviar.' },
			{ status: 429 }
		);
	}
  ...
  `;
	...
	const customerText = [
		...
		analysisMode === 'partial'
			? 'Nota: se ha generado un informe técnico parcial con los checks disponibles.'
			: '',
		'',
		...metricLines,
		'',
