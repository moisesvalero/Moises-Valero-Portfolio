<script lang="ts">
	...
	const title = $derived(
		isWhatsapp ? 'Perfecto, seguimos por WhatsApp' : 'Gracias, hemos recibido tu solicitud'
	);
...
<main class="min-h-screen bg-slate-50 px-6 py-16">
	<section
		class="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-8 md:p-12 shadow-[0_18px_40px_rgba(15,23,42,0.08)]"
	>
		<span
			class="inline-flex rounded-full bg-emerald-100 text-emerald-700 px-3 py-1 text-xs font-semibold uppercase tracking-wide"
		>
			Gracias
		</span>
