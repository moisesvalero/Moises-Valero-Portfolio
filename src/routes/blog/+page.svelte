<script lang="ts">
	...
	const categories = $derived([
		'Todos',
		...new Set(articles.map((article) => article.categoryLabel).filter(Boolean))
	]);
	let activeCategory = $state('Todos');
	const visibleArticles = $derived(
		activeCategory === 'Todos'
			? articles
			: articles.filter((article) => article.categoryLabel === activeCategory)
	);
	...
	const editorialArticles = $derived(
		recommendedArticles.length ? recommendedArticles : articles.slice(0, 3)
	);
...
<main class="blog-index">
	<section class="blog-hero" aria-labelledby="blog-title">
		...
		<div class="blog-shell blog-hero-shell">
			...
			<p class="hero-lead">
				Recursos prácticos sobre desarrollo web, rendimiento, seguridad, SEO técnico e IA aplicada a
				proyectos reales.
			</p>
		</div>
	</section>
...
<style>
	...
	.article-card {
		...
		border-radius: 8px;
		background:
			linear-gradient(180deg, rgba(255, 255, 255, 0.96), rgba(248, 250, 252, 0.86)), #ffffff;
		color: inherit;
		text-decoration: none;
		box-shadow:
