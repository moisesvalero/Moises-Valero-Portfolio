/** Tailwind solo para /diseno-web-alcoy (utilidades con important: #alcoy-landing-root). */
const pluginForms = require('@tailwindcss/forms');
const pluginContainerQueries = require('@tailwindcss/container-queries');

/** @type {import('tailwindcss').Config} */
module.exports = {
	darkMode: 'class',
	important: '#alcoy-landing-root',
	corePlugins: {
		preflight: false
	},
	content: [
		'./src/routes/diseno-web-alcoy/**/*.{html,js,svelte,ts}',
		'./src/lib/components/HeaderBrand.svelte',
		'./src/lib/components/JsonLdScript.svelte',
		'./src/lib/components/landing/**/*.{html,js,svelte,ts}'
	],
	theme: {
		extend: {
			colors: {
				'on-primary-fixed': '#001b3c',
				'surface-variant': '#e0e3e5',
				'on-primary': '#ffffff',
				'on-secondary-fixed': '#002113',
				'primary-container': '#1a365d',
				'outline-variant': '#c4c6cf',
				'surface-bright': '#f7f9fb',
				'surface-dim': '#d8dadc',
				'inverse-primary': '#adc7f7',
				'tertiary-container': '#003a55',
				'on-error': '#ffffff',
				'tertiary-fixed': '#c9e6ff',
				'secondary-fixed-dim': '#4edea3',
				'tertiary-fixed-dim': '#89ceff',
				'surface-container': '#eceef0',
				'secondary-container': '#6cf8bb',
				'inverse-surface': '#2d3133',
				'on-surface-variant': '#43474e',
				'primary-fixed': '#d6e3ff',
				'secondary-fixed': '#6ffbbe',
				'on-tertiary': '#ffffff',
				surface: '#f7f9fb',
				'on-surface': '#191c1e',
				'on-secondary-container': '#00714d',
				tertiary: '#002336',
				secondary: '#006c49',
				primary: '#002045',
				'surface-container-highest': '#e0e3e5',
				'on-error-container': '#93000a',
				error: '#ba1a1a',
				'inverse-on-surface': '#eff1f3',
				'surface-container-high': '#e6e8ea',
				'on-primary-container': '#86a0cd',
				'surface-container-lowest': '#ffffff',
				'primary-fixed-dim': '#adc7f7',
				'on-tertiary-fixed': '#001e2f',
				'on-primary-fixed-variant': '#2d476f',
				'surface-tint': '#455f88',
				outline: '#74777f',
				'on-tertiary-container': '#1ba9ed',
				'surface-container-low': '#f2f4f6',
				'on-background': '#191c1e',
				'on-tertiary-fixed-variant': '#004c6e',
				'on-secondary': '#ffffff',
				'on-secondary-fixed-variant': '#005236',
				background: '#f7f9fb',
				'error-container': '#ffdad6'
			},
			fontFamily: {
				headline: ['Manrope Variable', 'Manrope', 'system-ui', 'sans-serif'],
				body: ['Inter Variable', 'Inter', 'system-ui', 'sans-serif'],
				label: ['Inter Variable', 'Inter', 'system-ui', 'sans-serif']
			},
			borderRadius: { DEFAULT: '0.125rem', lg: '0.25rem', xl: '0.5rem', full: '0.75rem' }
		}
	},
	plugins: [pluginForms, pluginContainerQueries]
};
